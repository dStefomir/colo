...
float Star(vec2 uv, float flare){
    ...
    float m = .05/d;
    float rays = max(0., 1.-abs(uv.x*uv.y));
    m +=rays*flare;

    uv *=Rot(3.1415/4.);
    rays = max(0., 1.-abs(uv.x*uv.y));
    m +=rays*.3*flare;
    m *=smoothstep(1., .2, d);
    return m;
}
...
vec3 StarLayer(vec2 uv){
    ...
    for(int y=-1; y<=1; y++){
        for(int x=-1; x<=1; x++){
            ...
            float star= Star(gv-offset-(vec2(n, fract(n*34.))-.5), smoothstep(.9, 15, size));
...
vec4 mainImage( in vec2 fragCoord )
{
    ...
    float t=  iTime*.02;
    vec2 M = (vec2(0,0).xy-iResolution.xy*.5)/iResolution.y;
    uv *=Rot(t);
    uv +=M*4.;
